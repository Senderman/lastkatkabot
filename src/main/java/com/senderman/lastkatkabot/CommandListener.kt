...
class CommandListener internal constructor(private val handler: LastkatkaBotHandler) {
    private val usercommands: UsercommandsHandler = UsercommandsHandler(handler)
    private val adminCommands: AdminHandler = AdminHandler(handler)
    private val tournamentHandler: TournamentHandler = handler.tournamentHandler
    ...
    fun row(message: Message) {
        ...
        var oldRow: UserRow? = null
        if (handler.userRows.containsKey(message.chatId)) oldRow = handler.userRows[message.chatId]
        try {
        ...
        if (oldRow != null) handler.userRows.remove(message.chatId, oldRow)
